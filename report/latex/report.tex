...
A classe \texttt{UI.java} foi gerada através de um prompt de IA que pedia explicitamente a criação de uma interface de
linha de comando para o cliente, com um menu textual que incluísse operações de autenticação, registo de eventos,
consulta de agregações estatísticas e gestão de notificações. O prompt indicava ainda que a interface deveria utilizar a
biblioteca cliente já implementada, respeitando o protocolo e os códigos de erro definidos. O código produzido pela IA
serviu como base inicial e foi ajustado manualmente para alinhar a experiência de utilizador com o comportamento efetivo
do servidor.

\subsection{Geração das Classes de Teste}
As classes \texttt{FeatTest.java} e \texttt{StressTest.java} foram geradas com o apoio de IA, a partir de um prompt que
descrevia os cenários de teste desejados: verificação de operações básicas (registo, login, inserção de eventos,
consultas de agregação e notificações) e um teste de carga com dezenas de milhares de inserções concorrentes. A IA
produziu esboços de testes JUnit que foram posteriormente revistos, simplificados e complementados com asserts
específicos ao protocolo de mensagens do sistema.

\subsection{Acesso Rápido à Documentação de API}
A IA foi também utilizada como atalho para consulta de documentação de API da biblioteca padrão de Java. Num desses
momentos, ao procurar uma forma simples de implementar uma cache LRU, a IA chamou a atenção para o método
\texttt{removeEldestEntry()} da classe \texttt{LinkedHashMap}. Este detalhe, que poderia facilmente passar despercebido
numa leitura manual da documentação, revelou-se essencial: permitiu configurar a estrutura de dados de modo a que a
remoção do elemento menos recentemente utilizado fosse feita automaticamente, evitando a necessidade de implementar uma
estrutura LRU de raiz e simplificando significativamente o código da camada de persistência em memória.
