...
% --- CAPA ---
    ...
    concorrentemente sem bloqueio de cabeça de linha. A thread de leitura é nomeada e monitorizada para garantir que
    falhas de comunicação são detetadas e propagadas a todas as operações pendentes.
    ...
    \subsection{Tolerância a Falhas de Comunicação}
    Para resistir a falhas de comunicação inevitáveis em redes reais, o sistema implementa timeouts em dois níveis. No
    socket, cliente e servidor configuram \texttt{SO\_TIMEOUT} para detetar rapidamente conexões mortas ou bloqueadas.
    Na aplicação, operações normais expiram após 30 segundos, enquanto notificações bloqueantes aguardam indefinidamente.
    Este desenho evita que buffers congestionados paralizem o sistema inteiro, garantindo que ambas as partes se recuperam
    automaticamente quando a rede normaliza.
